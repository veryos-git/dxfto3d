...
let profileName = s_argument.split('/').pop().replace(/\.dxf$/i, '');
// Make it a valid OpenSCAD identifier (replace spaces/dashes with underscores)
profileName = profileName.replace(/[^a-zA-Z0-9_]/g, '_');
...
function ${profileName}_xpositive(scalefactor=1) = [
...
function ${profileName}_mirroredx(scalefactor=1) =
    let(half = ${profileName}_xpositive(scalefactor))
...
function ${profileName}_mirroredx_rotatedz(scalefactor=1) =
    let(full = ${profileName}_mirroredx(scalefactor))
    [for (p = full) [p.y, -p.x]];
...
function ${profileName}_for_revolve_around_x(scalefactor=1) =
    let(full = ${profileName}_mirroredx(scalefactor))
...
module ${profileName}_revolve_around_x(scalefactor=1, angle=90) {
    // Calculate the shift amount to undo it after extrusion
    full = ${profileName}_mirroredx(scalefactor);
    ...
    polygon(${profileName}_for_revolve_around_x(scalefactor));
}

// ===== PROFILE BOUNDING BOX =====
// Calculated from the half profile, used for spacing test parts
${profileName}_width = ${(maxX - minX).toFixed(6)};   // X extent of half profile
${profileName}_height = ${(maxY - minY).toFixed(6)};  // Y extent of profile

// Full profile dimensions (mirrored)
${profileName}_full_width = ${profileName}_width * 2;
${profileName}_full_height = ${profileName}_height;

// Spacing with margin (2x profile dimensions for comfortable viewing)
spacing_y = ${profileName}_full_height * 2 + 10;
spacing_x = ${profileName}_full_width * 2 + 10;
...
color("red")
path_sweep(${profileName}_mirroredx(1), test_line);

translate([0, spacing_y, 0])
color("green")
path_sweep(${profileName}_xpositive(1), test_line);

translate([0, spacing_y * 2, 0])
color("blue")
path_sweep(${profileName}_mirroredx_rotatedz(1), test_line);

// Test revolve around X axis (90 degree turn)
translate([0, spacing_y * 3, 0])
color("purple")
${profileName}_revolve_around_x(1, 90);

// Show the 2D profiles for reference
translate([test_length + 20, 0, 0]) {
    color("red", 0.5)
    linear_extrude(1)
    polygon(${profileName}_mirroredx(1));

    translate([0, spacing_y, 0])
    color("green", 0.5)
    linear_extrude(1)
    polygon(${profileName}_xpositive(1));

    translate([0, spacing_y * 2, 0])
    color("blue", 0.5)
    linear_extrude(1)
    polygon(${profileName}_mirroredx_rotatedz(1));
}
`;

// Write the SCAD file
let outputPath = s_argument.replace(/\.dxf$/i, '_profile.scad');
await Deno.writeTextFile(outputPath, s_scad);
console.log(`\nOpenSCAD profile file saved to: ${outputPath}`);
...
let svgOutputPath = s_argument.replace(/\.dxf$/i, '_profile.svg');
