...
function ppbig_xpositive(scalefactor=1) = [
...
function ppbig_mirroredx(scalefactor=1) =
    let(half = ppbig_xpositive(scalefactor))
...
function ppbig_mirroredx_rotatedz(scalefactor=1) =
    let(full = ppbig_mirroredx(scalefactor))
    [for (p = full) [p.y, -p.x]];
...
function ppbig_for_revolve_around_x(scalefactor=1) =
    let(full = ppbig_mirroredx(scalefactor))
...
module ppbig_revolve_around_x(scalefactor=1, angle=90) {
    // Calculate the shift amount to undo it after extrusion
    full = ppbig_mirroredx(scalefactor);
    ...
    polygon(ppbig_for_revolve_around_x(scalefactor));
}

// ===== PROFILE BOUNDING BOX =====
// Calculated from the half profile, used for spacing test parts
ppbig_width = 4.000000;   // X extent of half profile
ppbig_height = 4.417234;  // Y extent of profile

// Full profile dimensions (mirrored)
ppbig_full_width = ppbig_width * 2;
ppbig_full_height = ppbig_height;

// Spacing with margin (2x profile dimensions for comfortable viewing)
spacing_y = ppbig_full_height * 2 + 10;
spacing_x = ppbig_full_width * 2 + 10;
...
color("red")
path_sweep(ppbig_mirroredx(1), test_line);

translate([0, spacing_y, 0])
color("green")
path_sweep(ppbig_xpositive(1), test_line);

translate([0, spacing_y * 2, 0])
color("blue")
path_sweep(ppbig_mirroredx_rotatedz(1), test_line);

// Test revolve around X axis (90 degree turn)
translate([0, spacing_y * 3, 0])
color("purple")
ppbig_revolve_around_x(1, 90);

// Show the 2D profiles for reference
translate([test_length + 20, 0, 0]) {
    color("red", 0.5)
    linear_extrude(1)
    polygon(ppbig_mirroredx(1));

    translate([0, spacing_y, 0])
    color("green", 0.5)
    linear_extrude(1)
    polygon(ppbig_xpositive(1));

    translate([0, spacing_y * 2, 0])
    color("blue", 0.5)
    linear_extrude(1)
    polygon(ppbig_mirroredx_rotatedz(1));
}
