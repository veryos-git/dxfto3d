...
// Place revolved profiles at specific points (legacy - uses width/chamfer params)
...
// Place revolved profiles at arc endpoints (legacy - uses width/chamfer params)
...
// Place revolved profiles at specific points (takes profile directly)
module place_joints_at_points_with_profile(joint_profile) {
${Array.from(allPoints.values()).map(data => {
  return `    translate(point${data.index}) revolved_profile(joint_profile);`;
}).join('\n')}
}

// Place revolved profiles at arc endpoints (takes profile directly)
module place_joints_at_arc_points_with_profile(joint_profile) {
${a_o_arc.map((arc, arc_idx) => {
...
// Parameters:
//   profile - 2D profile points for sweeping (e.g., pyramid_profile_3_1_chamf_points(3, 0.8))
//   joint_profile - 2D half-profile for joints (e.g., pyramid_profile_half(3, 0.8))
...
module final_part(profile, joint_profile, render_joints=true, render_arc_joints=true) {
    union() {
        // Sweep the paths with the provided profile
        sweep_pattern(profile);

        // Place revolved joints at all connection points (if enabled)
        if (render_joints) {
            place_joints_at_points_with_profile(joint_profile);
        }

        // Place revolved joints at arc endpoints (if enabled)
        if (render_arc_joints) {
            place_joints_at_arc_points_with_profile(joint_profile);
        }
    }
}
...
module part(profile_width, profile_chamfer) {
    final_part(
        ...
        pyramid_profile_half(profile_width, profile_chamfer)
    );
}
    ...
    let(
        ...
        difference(){
            ...
            final_part(
                ...
                pyramid_profile_half(wgroove, cgroove)
            );
            ...
            union(){
                ...
                for (x=[0:xitems-1]){
                    for (y=[0:yitems-1]){
                        ...
                        color("blue")
                        final_part(
                            pyramid_profile_3_1_chamf_points(w, c),
                            pyramid_profile_half(w, c)
                        );
                    }
                }
            }
